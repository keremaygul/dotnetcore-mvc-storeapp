...
namespace StoreApp.Areas.Admin.Controllers
{
    ...
    public class UserController : Controller
    {
        ...
        {
            var user = await _manager.AuthService.GetOneUserForUpdate(id); // id olarak yazıyo ama userName kullanılacak
            return View(user);
        }
        ...
        {
            if (!ModelState.IsValid)
                return View(model); // Model doğrulama hatalarını geri döndür.

            var result = await _manager.AuthService.ResetPassword(model);
            if (result.Succeeded)
                return RedirectToAction("Index");

            // Hataları döngüyle ekleyin
            ...
            return View(model);
        } // fixlendi çünkü error görünmesi gerekiyordu
